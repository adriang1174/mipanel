...
define('CODIGO_COMERCIO_MX', '34115');

//(!)revisar
class p_gateways_dinero_mail_mx_ticket {
    
    function p_gateways_dinero_mail_mx_ticket($ticketid) {
...
class p_gateways_dinero_mail_mx extends p_gateways {

	function p_gateways_dinero_mail_mx($userid) {
    ...
    function get_ticket($ticketid, $amount2pay = 0) {
		...
		*/
        ...
        $cot_medio = ca_currency::get_last_quotation_by_id( 3 ); // PESOS MX
        $cot = $cot_fact['cotizacion'] / $cot_medio['cotizacion'];
		
		$p_dinero_mail_mx->total_pesos = round(($p_dinero_mail_mx->total * $cot), 2);
		$p_dinero_mail_mx->total_pesos_string = number_format(($p_dinero_mail_mx->total * $cot), 2);
		
        		/* VARIABLES OBLIGATORIAS PARA LA LLAMADADA */

		$p_dinero_mail_mx->url_action = 'https://mexico.dineromail.com/Shop/Shop_Ingreso.asp';
		$p_dinero_mail_mx->NombreItem = 'pago total/parcial ' . $p_dinero_mail_mx->ticketid;
		$p_dinero_mail_mx->TipoMoneda = "1"; // PESOS MX
		
		
		$p_dinero_mail_mx->E_Comercio = CODIGO_COMERCIO_MX; //CODIGO_COMERCIO;
